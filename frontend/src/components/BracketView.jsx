  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        {bracket.kind === "double" && (
          <span
            data-testid="bracket-kind-badge"
            className="font-mono-data text-[10px] uppercase tracking-widest text-amber-800 bg-amber-100 border border-amber-300 rounded-full px-2 py-0.5"
          >
            Double elim
          </span>
        )}
      ...
      {bracket.kind === "double" ? (
        <div className="space-y-6" data-testid="bracket-double">
          <BracketStream
            label="Winners bracket"
            testid="bracket-wb"
            tiers={bracket.wb_tiers || []}
            finalLabel="WB Final"
            {...{ nameFor, isDirector, report, reporting }}
          />
          {(bracket.lb_tiers || []).length > 0 && (
            <BracketStream
              label="Losers bracket"
              testid="bracket-lb"
              tiers={bracket.lb_tiers || []}
              finalLabel="LB Final"
              accent="rose"
              {...{ nameFor, isDirector, report, reporting }}
            />
        ...
        <div className="flex gap-4 overflow-x-auto pb-2">
          {(bracket.tiers || []).map((tier, tIdx) => (
            ...
            >
              ...
              {tier.map((m) => renderMatch(m, { nameFor, isDirector, report, reporting }))}
            </div>
          ))}
        </div>
      )}
...
function renderMatch(m, { nameFor, isDirector, report, reporting }) {
...
function BracketStream({ label, testid, tiers, finalLabel, accent = "slate", nameFor, isDirector, report, reporting }) {
  const accentTitle = {
  ...
  return (
    <div data-testid={testid}>
      <div className={`font-display text-sm mb-2 ${accentTitle}`}>{label}</div>
      <div className="flex gap-4 overflow-x-auto pb-2">
        {tiers.map((tier, tIdx) => (
          <div
            key={tIdx}
            className="min-w-[240px] flex flex-col gap-3"
            data-testid={`${testid}-tier-${tIdx}`}
          >
            <div className="font-mono-data text-[10px] uppercase tracking-widest text-slate-500 pb-1 border-b border-slate-200">
              {tIdx === tiers.length - 1 ? finalLabel : `Tier ${tIdx + 1}`}
            </div>
            {tier.map((m) => renderMatch(m, { nameFor, isDirector, report, reporting }))}
          </div>
        ))}
